...
if (rave >= 0) {
  ...
  e.pt_launch(rave);
  check("RAVE requests its own music track", e.pt_music_track() === 6);
  ...
  check("RAVE short click selects TECHNO STYLE", e.pt_music_track() === 7);
  tick(0, 0, BTN_CLICK); tick();
  check("RAVE short click selects DODGEMS", e.pt_music_track() === 8);
  tick(0, 0, BTN_CLICK); tick();
  check("RAVE song selection wraps to MECHANICAL", e.pt_music_track() === 6);
  for (let frame = 0; frame < 32; frame++) tick(0, 0, BTN_CLICK);
  tick();
  check("RAVE held click does not change songs", e.pt_music_track() === 6);
  const raveFrame = ({
